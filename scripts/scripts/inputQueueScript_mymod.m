...
function [muxPattern] = inputQueueScript_mymod(block)

  retval = 0;
  %block = 1;
  muxPattern = inputQueueMuxPattern_mymod(block); # 52 x 2 x 20
  fifoRows = 32;
  fifoColumns = 17;
  muxOutSymbols = size(muxPattern)(1);
  sliceAddressWidth = ceil(log2(size(muxPattern)(3))); %5 bits for 2^5=32>20
  READDISABLEDCASE = (2^sliceAddressWidth)-1;
  datawidth = 6;
  
  filename = sprintf("./outputs/InputQueueCirc_%d.v",block-1);
  ...
  fprintf(fid,sprintf(");\n"));
  fprintf(fid,sprintf("parameter w = %d; // Width \n",datawidth));
  fprintf(fid,sprintf("parameter maxVal = %d; // These are 0 LLRs \n",( (2^datawidth)-1) ));
  ...
  fprintf(fid,sprintf("input [ADDRESSWIDTH-1:0]sliceAddress;\n"));
  fprintf(fid,sprintf("input wr_en,rd_en;\n input clk,rst;\n\n"));
 
  fprintf(fid,sprintf("wire [ADDRESSWIDTH-1:0]sliceAddress_case;\n")); 
...
%  fprintf(fid,sprintf("        end\n"));
  ...
  fprintf(fid,sprintf("assign sliceAddress_case = rd_en ? sliceAddress : READDISABLEDCASE;\n\n")); 
  
  fprintf(fid,sprintf("// MUX WIRING\n"));
  fprintf(fid,sprintf("always@(*)begin\n"));
  fprintf(fid,sprintf("    case(sliceAddress_case)\n"));
  ...
  sprintf("inputQueueScript_mymod function completed")
