...
MB = size(Gne_p)(2)/Z
%generate random msg:
mZ=gf([zeros(1,18),randi([0,1],1,k)],1);
...
for circ=1:KB
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %for the first circulant since last cycle is avoided an extra lm schift is given to func gen
  if(circ==1)
  circrow = func_rotator(circrow,Lm,'R');
  end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%for first circulant last 16 bits are not used
if(circ==1)
   for i=1:Lm:size(m_circz)(2)-Lm
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %for loop MAC-mul is same as above matrix based MAC-mul.
...
pExpected = mZ*GZ;
pObserved  = pr(blkcol,:);
%pObserved = func_rotator(pr(blkcol,:),2,'R'); %make this correction in f
